      ...
      foreach (Object targetObject in targets)
      {
        ...
        if (meshSimplify.m_meshSimplifyRoot == null && meshSimplify.m_bEnablePrefabUsage)
        {
          ...
          if (string.IsNullOrEmpty(strMeshAssetPath))
          {
            ...
            strMeshAssetPath = UnityEditor.EditorUtility.SaveFilePanelInProject("Save mesh asset(s)", "mesh_" + gameObject.name + gameObject.GetEntityId().ToString() + ".asset", "asset", "Please enter a file name to save the mesh asset(s) to");
